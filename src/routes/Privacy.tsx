...
export default function Privacy() {
  const { t } = useI18n();
  useEffect(() => applyMeta(seo.privacy), []);
  return (
    <main className="relative z-10 min-h-screen px-6 sm:px-10 py-32">
      <div className="max-w-2xl mx-auto">
        ...
        <h1 className="mt-8 font-display font-bold text-4xl sm:text-5xl">{t.privacy.title}</h1>
        <p className="mt-8 text-[color:var(--cream)]/80 leading-relaxed">{t.privacy.body}</p>
      </div>
    </main>
  );
}
