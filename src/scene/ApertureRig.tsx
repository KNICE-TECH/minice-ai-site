...
export function ApertureRig({ lowEnd = false }: { lowEnd?: boolean }) {
  ...
  return (
    <group ref={ref} position={[0, 0.1, 0]} scale={0.95}>
      <Backdrop />
      <LightBeams />
      <Blade index={0} color="#f5f4f1" lowEnd={lowEnd} />
      <Blade index={1} color="#8a8a87" lowEnd={lowEnd} />
      <Blade index={2} color="#e8a87c" lowEnd={lowEnd} />
    </group>
  );
}
